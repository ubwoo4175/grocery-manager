...
const RecipeFormSchema = z.object({
  ...
  // Add fields for the new ingredient row to the schema for validation
...
const NewIngredientSubSchema = z.object({
    ...
    quantity: z.number({ message: "Must be a number." }).min(0).refine((val) => /^\d+(\.\d{1})?$/.test(String(val)), { message: "Max one decimal." }),
    unit: z.string().min(1, "Unit is required."),
});


// Zod schema with async validation for submission
const createRecipeFormSchemaForSubmit = (recipeId?: string) =>
      ...
      <div className="container mx-auto max-w-3xl">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="bg-white p-8 rounded-xl shadow-md space-y-8">
            {/* Fields for existing ingredients... */}
            <div>
              ...
              <div className="grid grid-cols-12 gap-2 items-start">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-5">
                      <FormControl>
                        <Input placeholder="New Ingredient Name" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-3">
                      <FormControl>
                        <Input type="number" step="0.1" placeholder="Qty" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : parseFloat(e.target.value))} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
